...
//     // Use a dedicated environment variable for the main frontend URL
...
const generateInsight = (poll) => {
  ...
  const secondOption = sortedOptions[1];
  const topOptionPct = Math.round((topOption.votes / totalVotes) * 100);
  if (topOption.votes === secondOption.votes) { return `The vote is currently tied between "${topOption.text}" and "${secondOption.text}".`; }
  const margin = topOption.votes - secondOption.votes;
  if ((margin / totalVotes) > 0.10) { return `"${topOption.text}" is the clear favorite, securing ${topOptionPct}% of the votes.`; }
  else { return `It's a close race, but "${topOption.text}" has a slight edge with ${topOptionPct}%.`; }
};
...
export const createPoll = async (req, res) => {
  try {
    ...
    const baseUrl = process.env.FRONTEND_URL;
    const pollUrl = `${baseUrl}/poll/${newPoll._id}`;
    res.status(201).json({ pollId: newPoll._id, pollUrl });
  } catch (error) {
    console.error("Error in createPoll:", error); // <-- Logging is here
    res.status(500).json({ message: 'Server error creating poll.' });
  }
};
  ...
  try {
    ...
    if (voteToken) {
      try {
        const decoded = jwt.verify(voteToken, process.env.JWT_SECRET);
        userVote = decoded.optionId;
      } catch (err) { /* Invalid token, ignore */ }
    }
    res.json({ ...poll.toObject(), totalVotes, insight, userVote });
  } catch (error) {
    console.error("Error in getPoll:", error); // <-- Logging is here
    res.status(500).json({ message: 'Server error fetching poll.' });
  }
};
...
export const addVote = async (req, res) => {
  try {
    ...
    if (new Date() > poll.expiresAt) { return res.status(400).json({ message: 'This poll has expired.' }); }
    if (req.cookies[`poll_vote_${pollId}`]) { return res.status(403).json({ message: 'You have already voted in this poll.' }); }
    const userAgent = req.headers['user-agent'] || 'unknown';
    const ip = req.ip || req.connection.remoteAddress;
    const fingerprint = crypto.createHash('sha256').update(ip + userAgent + pollId).digest('hex');
    if (poll.voterFingerprints.includes(fingerprint)) { return res.status(403).json({ message: 'You have already voted in this poll.' }); }
    const option = poll.options.id(optionId);
    ...
    console.error("Error in addVote:", error); // <-- Logging is here
    res.status(500).json({ message: 'Server error processing vote.' });
  }
};